...
const BlogCardCopy = ({
  ...
  return (
    <div className="card-wrapper shadow-lg rounded cursor-pointer">
      <div className=" text-black h-[400px]">
        ...
        <div className="content p-3">
          ...
          <Link to={`/blog/${_id}`}>
            <p className="text-black"> {excerpt(description)}see more</p>
          </Link>
          <Link to={`/userblog/${creator}`}>
            <p className="mb-2 text-sm text-gray-500">
              <FaUser className="inline-flex items-center mr-2" /> {name} (see
              other blogs this user)
            </p>{" "}
          </Link>
        </div>
      </div>
      <div className="last flex justify-between place-items-center p-2">
        <MDBBtn
          style={{ float: "right" }}
          tag="a"
          color="none"
