...
const Sidebar = () => {
  ...
  const { latestblog, loading } = useSelector((state) => state.blog);
  ...
  return (
    <div>
      ...
      <div>
        <h3 className="font-bold mt-5 mb-1 border-b-2 rounded-sm">
          Latest Blogs
        </h3>
        <div>
          {loading ? ( // Show loading state
            ...
            latestblog?.slice(0, 7).map((latest) => (
              <div key={latest._id} className="">
                <div>
                  <img src={latest.imageFile} alt="" className="w-full" />
                </div>
                <div>
                  ...
                  <Link
                    to={`/blog/${latest?._id}`}
                    className="font-medium py-1 hover:text-orange-500"
                  >
