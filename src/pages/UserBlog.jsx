...
const UserBlog = () => {
  ...
  return (
    <div className="mt-7">
      {loading ? ( // লোডিং স্টেট দেখানোর জন্য
          ...
          {userblog.length > 0 ? (
            <div>
              <h1 className="text-2xl font-bold text-center">
                Users Blog: {userBlogthisuser?.[0]?.name}
              </h1>
              <hr className="w-2/4 mx-auto" />
            </div>
          ) : (
            <h1 className="text-center">
              No Blogs available with the user: {userBlogthisuser?.[0]?.name}
            </h1>
          )}
          {userblog?.length > 0 &&
            userblog?.map((blog) => (
              <div
                key={blog._id}
                className="flex flex-col lg:flex-row justify-between gap-3 items-center w-2/4 mx-auto p-5 shadow-lg rounded my-2"
              >
                <img
                  src={blog?.imageFile}
                  alt=""
                  className="lg:w-44 w-36 h-36 lg:h-44"
                />
                <div>
                  <h1 className="text-sm text-black lg:font-bold">
                    {blog?.title}
                  </h1>
